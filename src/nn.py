...
class searchnet:
    ...
    def get_strength(self, fromid ,toid ,layer):
        ...
        if res is None:
            if layer==0: return -0.2 # the nn default value of wordlayer to hiddenlayer
            else: return 0
        return res[0]
    ...
    def generate_hidden_node(self, wordids, urls):
        ...
        if res is None:
            cur=self.con.execute("insert into hiddennode (create_key) values ('%s')"
                                 % create_key)
            hiddenid=cur.lastrowid # the method to get primary key after a insert operation
    ...
    def get_all_hidden_ids(self, wordids, urlids):
        ...
        for urlid in urlids:
            cur = self.con.execute('select fromid from hiddenurl where toid=%d'
                                   % urlid)
            for row in cur: l1[row[0]]=1
        return list(l1.keys()) # there was a bug, dict,keys() return a iteralble not a list
    ...
    def set_up_network(self, wordids, urlids):
        ...
        #[1.0]*3 means [1.0, 1.0, 1.0]
        self.ah=[1.0]*len(self.hiddenids)
        self.ao=[1.0]*len(self.urlids)

        # create wights matrix
        # weight matrix on input-hidden layer
        self.wi=[[self.get_strength(wordid,hiddenid,0)
                  for hiddenid in self.hiddenids]
                  for wordid in self.wordids]
        # weight matrix on hidden-output layer
    ...
    def feed_forward(self):
        ...
        return self.ao[:]# means ao all elements
